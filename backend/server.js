...
app.post('/register', async (req, res) => {
  ...
  if (secretKey !== process.env.SECRET_KEY) return res.status(403).json({ message: "Invalid secret key" });
...
app.post('/api/news', async (req, res) => {
  ...
  const newNews = new News({ title, text, images, tags, author });
  await newNews.save();
  io.emit('newsCreated', newNews); // Emit event to all connected clients
  res.json(newNews);
});

// Get News with Pagination (Infinite Scroll)
// app.get('/api/news', async (req, res) => {
//   const { page = 1 } = req.query;
//   const news = await News.find().sort({ createdAt: -1 }).skip((page - 1) * 3).limit(3);
//   res.json(news);
// });
...
app.get('/api/news/tags/:tag', async (req, res) => {
  ...
  const { page = 1 } = req.query;
  const news = await News.find({ tags: tag }).sort({ createdAt: -1 }).skip((page - 1) * 3).limit(3);
  res.json(news);
});
